...
- **Install command** — what a fresh clone needs before it can build or typecheck: `bun install`, `npm ci`, `cargo fetch`, `uv sync`, `go mod download`, and so on. Record an empty value for repositories that need no install step.
