...
Triage pending issue tickets in `.tars/issues/todo/` (conforming to the template in [tars-backlog-create-issue](../tars-backlog-create-issue/SKILL.md)) to verify their accuracy, identify implementation gaps, check for hallucinations, and append a detailed review section to each.
...
- **Prompt**:
  ...
  2. Implementation Readiness Check:
     ...
     - Is the `files:` frontmatter present, non-empty, and complete? It is a primary input to the parallel-batching File rule, so an omission is not cosmetic: the ticket either gets serialised or gets batched into a collision. Check the ticket body for files the work will clearly touch that the list omits - tests, fixtures, barrel re-exports, lockfiles, shared implementation modules - and report them.
     - Should `owns:` list a shared module or `path#symbol` (e.g. a constant two tickets might both introduce)? If the body clearly claims ownership of an export or shared helper, require or suggest `owns:` entries so implement can serialise collisions the File rule would miss.
     - Soft dependencies: if this ticket needs an export or behaviour another open ticket will add, require a `dependencies:` edge (or report the gap) even when `files:` do not overlap.
