...
- **Prompt**:
  ...
  2. Implementation Readiness Check:
     ...
     - Is the `files:` frontmatter present, non-empty, and complete? This list is the sole input to the parallel-batching File rule, so an omission is not cosmetic: the ticket either gets serialised or gets batched into a collision. Check the ticket body for files the work will clearly touch that the list omits — tests, fixtures, barrel re-exports, lockfiles — and report them.
