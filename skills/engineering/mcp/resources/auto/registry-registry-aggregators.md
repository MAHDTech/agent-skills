...
The `"deleted"` status typically indicates that a server has violated our permissive [moderation policy](registry-moderation-policy.md), suggesting the server might be spam, malware, or illegal. Aggregators may prefer to remove these servers from their index.
