...
[Extensions](https://modelcontextprotocol.io/extensions/overview) are where convergence gets tested; the specification is where it gets committed.
...
When someone asks why MCP doesn't support a feature directly, the answer is usually that it can be built from what MCP already provides. Extensions like [MCP Apps](https://modelcontextprotocol.io/extensions/apps/overview) and [Tasks](https://modelcontextprotocol.io/extensions/tasks/overview) capture the patterns that emerge.
...
We encourage the use of [MCP extensions](https://modelcontextprotocol.io/extensions/overview) as a way to experiment with new patterns that may eventually lead to standardization.