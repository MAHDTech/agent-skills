...
After [generating client bindings](/docs/clients/codegen) for your
module, you can establish a connection to your SpacetimeDB
[database](/docs/databases) from your client application. The
...
2.  A published database running on SpacetimeDB (local or on
    [MainCloud](/docs/how-to/deploy/maincloud))
...
documentation](/docs/core-concepts/authentication/spacetimeauth/) for
...
[identity](/docs/intro/key-architecture#identity) from the server.
...
The [identity](/docs/intro/key-architecture#identity) persists across
connections and represents the user, while the [connection
ID](/docs/intro/key-architecture#connectionid) is unique to each
connection session.
...
- [Use the SDK API](/docs/clients/api) to interact with tables, invoke
