...
serially, an agent can delegate tasks—such as running tests or
...
- **Workspace Options**: The subagent can either inherit the same
...
- **Monitoring**: You can directly monitor the progress of any subagent
...
- **`research`**: Optimized for codebase research, file navigation, and
  structural exploration.
- **`browser`**: Operates sandboxed web browsers to perform interactive
  browser testing (invoked exclusively via the `/browser` slash
  command).
...
> **Known Issue (Tool Validation)**: Specifying an unmapped or
> misspelled tool name in the `tools` list may cause the subagent
> process to hang during execution. Please double-check exact tool names
> (such as `view_file` or `run_command`) when configuring custom
...
- **Cancellation**: You can cancel a running subagent by clicking **Stop
  Subagent** in the subagent panel (or pressing `k` in the CLI).
...
- **Re-awakening**: An idle agent automatically re-awakens to the
  *Running* state upon receiving a message from another agent.
...
- **Cleanup**: Any temporary Git worktrees generated for the subagent
...
- **Flexible Routing**: Agents can communicate with parent agents,
