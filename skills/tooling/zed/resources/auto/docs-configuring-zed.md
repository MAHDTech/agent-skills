...
You may also want to change your [theme](themes.md), configure your [key bindings](https://zed.dev/key-bindings.md), set up [tasks](https://zed.dev/tasks.md), or install [extensions](extensions.md).
...
- Description: Controls where the <code class="hljs">editor: rewrap</code> action is allowed in the current language scope
...
You can find the names of your currently installed extensions by listing the subfolders under the [extension installation location](https://zed.dev/extensions/installing-extensions.md#installation-location):
...
page (<code class="hljs">zed: extensions</code>) does this automatically.
...
See [Git documentation](https://zed.dev/git.md#diff-view-styles) for more details.
...
- Description: What to do when the <code class="hljs">editor: go to definition</code> action fails to find a definition
...
- Description: How to scroll the target into view when navigating to a definition or reference (e.g. <code class="hljs">editor: go to definition</code>, <code class="hljs">editor: go to type definition</code>, <code class="hljs">editor: find all references</code>).
...
- Description: Whether or not to enable Helix mode. Enabling `helix_mode` also enables `vim_mode`. See the [Helix documentation](https://zed.dev/helix.md) for more details.
- Setting: `helix_mode`
- Default: `false`
...
Run the <code class="hljs">icon theme selector: toggle</code> action in the command palette to see a current list of valid icon theme names.
...
- Description: Collects timing data for foreground and background executor tasks so they can be inspected via the <code class="hljs">zed: open performance profiler</code> action. Enabling this may lead to increased memory usage, hence it's disabled by default for regular builds.
...
- Description:
  ...
  - Using the <code class="hljs">project panel: open permanent</code> action
  - Editing the file
...
Run the <code class="hljs">theme selector: toggle</code> action in the command palette to see a current list of valid theme names.
...
Visit [AI Quick Start](https://zed.dev/ai/quick-start.md) under the AI section to learn more about AI setup.
...
See the [debugger page](https://zed.dev/debugger.md) for more information about debugging support within Zed.
...
- Description: Configure any number of settings profiles that are temporarily applied when selected from <code class="hljs">settings profile selector: toggle</code>.
- Setting: `profiles`
- Default: `{}`
...
To preview and enable a settings profile, open the command palette via <kbd class="keybinding">cmd-shift-p&#124;ctrl-shift-p</kbd> and search for <code class="hljs">settings profile selector: toggle</code>.
