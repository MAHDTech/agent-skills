...
<Note>
  We recommend migrating your `.npmrc` file to Bun's [`bunfig.toml`](https://bun.com/runtime/bunfig) format, which supports more
...
The equivalent `bunfig.toml` option is [`install.registry`](https://bun.com/runtime/bunfig#install-registry):
...
The equivalent `bunfig.toml` option is to add a key in [`install.scopes`](https://bun.com/runtime/bunfig#install-registry):
...
The equivalent `bunfig.toml` option is [`install.linkWorkspacePackages`](https://bun.com/runtime/bunfig#install-linkworkspacepackages):
...
The equivalent `bunfig.toml` option is [`install.exact`](https://bun.com/runtime/bunfig#install-exact):
...
The equivalent `bunfig.toml` option is [`install.dryRun`](https://bun.com/runtime/bunfig#install-dryrun):
...
The equivalent `bunfig.toml` option is [`install.cache`](https://bun.com/runtime/bunfig#install-cache):
...
Control how packages are laid out in `node_modules`. For compatibility with other package managers, Bun accepts both npm's `install-strategy` and pnpm/yarn's `node-linker`. See [isolated installs](https://bun.com/pm/isolated-installs) for how the hoisted and isolated layouts differ.
