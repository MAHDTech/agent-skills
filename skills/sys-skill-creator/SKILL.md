...
All skills MUST be prefixed with one of the following 5 categories to maintain repository organization:

- **`cmd-`**: CLI execution, workspace manipulation, and local scripts (e.g., `cmd-pr-description`).
- **`brain-`**: Cognitive reasoning, personas, and conversational rules (e.g., `brain-critical-thinking`).
- **`sys-`**: Tools managing the agent's own state or memory (e.g., `sys-skill-creator`).
- **`api-`**: Remote API interactions and authentications.
- **`gen-`**: Asset and media generation (e.g., `gen-mermaid`).
