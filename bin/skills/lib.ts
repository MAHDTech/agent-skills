...
export async function getSkills(): Promise<Skill[]> {
    ...
    for (const file of files.sort()) {
        ...
        if (!match || !match[1] || !match[2]) {
            yamlError = "Missing or malformed YAML frontmatter delimiters"
            content = raw
        } else {
            try {
...
export async function releaseLock() {
    try {
        const content = await fs.readFile(ANTIGRAVITY_LOCK_FILE, "utf-8")
        ...
        for (const skill of skills) {
            ...
            if (exists) {
                if (isSymlink) {
                    ...
                    conflicts++
                    log.warn(
                        `⚠️ Conflict: Destination ${dest} is a real file or directory and will not be overwritten.`
                    )
                    continue
                }
            }
