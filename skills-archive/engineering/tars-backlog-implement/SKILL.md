...
**Devenv:** if the project has `devenv.nix` / `devenv.yaml`, prepare already followed the [devenv](../../../skills/tooling/devenv/SKILL.md) skill when freezing commands. Implement does not restate secrets or CI env vars - it only runs `tars-gate` / `tars-spoke`.
...
- [devenv](../../../skills/tooling/devenv/SKILL.md) - only if you must understand how prepare built commands; implement does not re-enter devenv by hand when `run.env` is present.
