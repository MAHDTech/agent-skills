...
>
> Every frozen string that enters a devenv shell (`TARS_INSTALL_COMMAND`, `TARS_HOOK_COMMAND`, `TARS_TEST_COMMAND`, and `TARS_CI_COMMAND` if it enters one) **must** carry, per the [devenv](../../../skills/tooling/devenv/SKILL.md) skill's non-interactive shape:
>
> ```bash
...
- [devenv](../../../skills/tooling/devenv/SKILL.md) - when the project has `devenv.nix` / `devenv.yaml`, follow it to build non-interactive enter commands during step 4d.
