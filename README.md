...
- **[backlog-create-issue](skills/planning/backlog-create-issue/SKILL.md)** — Use when creating a new backlog issue/ticket in the `.tars/issues/todo/` directory, defining its YAML frontmatter, headings, tasks, acceptance criteria, evidence collection, and triage review expectations.
...
- **[backlog-prepare](skills/planning/backlog-prepare/SKILL.md)** — Prepare to run the backlog-loop by cleaning up orphaned git worktrees and branches, and ensuring the working tree is clean. Reach for this to reset the environment before starting a full backlog loop.
...
- **[backlog-review](skills/review/backlog-review/SKILL.md)** — Review the code implementation of a backlog ticket on a subagent branch, assessing compliance with spec/acceptance criteria and repo standards before merge. Reach for this during the implementation phase of the backlog loop.
...
- **[prek](skills/tooling/prek/SKILL.md)** — Run pre-commit hooks using the prek CLI tool. Differentiates between 'pre-commit' (the git hooks concept/lifecycle stage) and 'prek' (the actual binary command). Use when you need to run, configure, check, or troubleshoot git pre-commit hooks.
