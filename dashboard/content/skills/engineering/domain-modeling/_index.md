...
Create files lazily — only when you have something to write. If no `CONTEXT.md` exists, create one when the first term is resolved. If no `docs/adr/` exists, create it when the first ADR is needed. If your project already keeps glossaries or decision records elsewhere, follow that existing convention instead.
...
When a term is resolved, update `CONTEXT.md` right there. Don't batch these up — capture them as they happen. Use the format in [CONTEXT-FORMAT.md](@/skills/engineering/domain-modeling/resources/manual/CONTEXT-FORMAT.md).
...
If any of the three is missing, skip the ADR. Use the format in [ADR-FORMAT.md](@/skills/engineering/domain-modeling/resources/manual/ADR-FORMAT.md).
