+++
title = "docs-draft-tools-inspector-web"
[extra]
skill = false
...
**Network** and **Console** never appear together. Legacy and modern eras are described in [Protocol eras](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras).
...
* **Protocol Era**: `legacy` / `auto` / `modern`. See [Protocol eras](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras).
* **Log level per request**: the level a modern-era connection stamps on each outgoing request by default, or `off` to opt out (see [Logging](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras#logging)).
...
On modern-era servers this screen also shows mirrored `Mcp-Param-*` headers, excluded tools, and distinct `-32602` error panels, all covered in [Protocol eras](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras#tools-mirrored-headers-and-excluded-tools).
...
See [Recipes](https://modelcontextprotocol.io/docs/draft/tools/inspector/recipes#reviewing-an-mcp-app) for the CLI-first automated review flow.
...
* **Protocol**: the JSON-RPC transcript. Requests paired with responses, notifications inline, [MRTR](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras#multi-round-tool-results-mrtr) rounds grouped as one conversation, and spec errors rendered by class.
...
A driver (a script, a CI harness, or the CLI's [`--print-handoff`](https://modelcontextprotocol.io/docs/draft/tools/inspector/authorization#handing-off-from-the-web-client-to-the-cli)) can reach a *connected* Inspector with a single navigation:
...
See the [Hosting on a network](https://modelcontextprotocol.io/docs/draft/tools/inspector/recipes#hosting-on-a-network) recipe for the full matrix, and [Configuration](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration#web-backend-environment-variables) for the variables.
