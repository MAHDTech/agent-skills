+++
title = "docs-draft-tools-inspector-recipes"
[extra]
skill = false
...
`--transport` accepts `http` (Streamable HTTP) and `sse`. If the server is protected, see [Authorization](https://modelcontextprotocol.io/docs/draft/tools/inspector/authorization): no setup is needed in advance, because when the server answers `401` the Inspector runs the OAuth flow described there and retries the connection.

For an HTTP server, also decide its [protocol era](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras). The default is `legacy`; set `modern` or `auto` in Server Settings (or `protocolEra` in the catalog file) to exercise the 2026-07-28 behavior.
...
Import merges into the active [catalog](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration#choosing-servers)
...
Read the [session token](https://modelcontextprotocol.io/docs/draft/tools/inspector/web#the-session-token) from the container logs, or pin it with `-e MCP_INSPECTOR_API_TOKEN=<value>`.
...
Its `HEALTHCHECK` probes the web UI, so add `--no-healthcheck` when running `--cli` or `--tui` (neither has a web server). `<target>` below is an [ad-hoc target](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration#ad-hoc-targets): a positional stdio command, or `--server-url \<url\> --transport http`.
...
\<Steps\>
  ...
  <Step title="Lock it in with the CLI">
    ...
    CI](https://modelcontextprotocol.io/docs/draft/tools/inspector/cli#verify-a-server-in-ci) for the full
    command.
  </Step>
</Steps>
