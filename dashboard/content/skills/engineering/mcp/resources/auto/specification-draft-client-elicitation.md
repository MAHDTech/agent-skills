+++
title = "specification-draft-client-elicitation"
[extra]
skill = false
...
Servers **MAY** request information from a user during the processing of a client request, by sending an [`InputRequiredResult`](https://modelcontextprotocol.io/specification/draft/basic/patterns/mrtr#inputrequiredresult)
...
**Input request (delivered inside [`InputRequiredResult.inputRequests`](https://modelcontextprotocol.io/specification/draft/basic/patterns/mrtr#inputrequests)):**
...
\<Note\>
  **Important**: URL mode elicitation is *not* for authorizing the MCP client's
  ...
  authorization](https://modelcontextprotocol.io/specification/draft/basic/authorization)). Instead, it's used when the MCP
...
Elicitations do not require that the server maintain state about users with the [multi round-trip requests](https://modelcontextprotocol.io/specification/draft/basic/patterns/mrtr#multi-round-trip-requests) mechanism.

However, if state is stored, servers implementing elicitation **MUST** securely associate this state with individual users following the guidelines in the [security best practices](https://modelcontextprotocol.io/docs/draft/tutorials/security/security_best_practices) document. Specifically:

* State storage **MUST** be protected against unauthorized access
* For remote MCP servers, user identification **MUST** be derived from credentials acquired via [MCP authorization](https://modelcontextprotocol.io/specification/draft/basic/authorization) when possible (e.g. `sub` claim)
...
Authorization with external APIs enabled by URL mode elicitation is separate from [MCP authorization](https://modelcontextprotocol.io/specification/draft/basic/authorization). MCP servers **MUST NOT** rely on URL mode elicitation to authorize users for themselves.

#### Understanding the Distinction

* **MCP Authorization**: Required OAuth flow between the MCP client and MCP server (covered in the [authorization specification](https://modelcontextprotocol.io/specification/draft/basic/authorization))
...
2. **The MCP server MUST NOT use the client's credentials for the third-party service**: That would be [token passthrough](https://modelcontextprotocol.io/docs/draft/tutorials/security/security_best_practices#token-passthrough), which is forbidden
...
\<Note\>
  ...
  section](https://modelcontextprotocol.io/docs/draft/tutorials/security/security_best_practices#token-passthrough)
...
Instead, servers **SHOULD** follow [security best practices](https://modelcontextprotocol.io/docs/draft/tutorials/security/security_best_practices).

Non-normative examples:

* Incorrect: Treat user input like "I am [joe@example.com](mailto:joe@example.com)" as authoritative
* Correct: Rely on [authorization](https://modelcontextprotocol.io/specification/draft/basic/authorization) to identify the user
...
Typically identity verification is done by leveraging the [MCP authorization server](https://modelcontextprotocol.io/specification/draft/basic/authorization) to identify the user, through a session cookie or equivalent in the browser.
