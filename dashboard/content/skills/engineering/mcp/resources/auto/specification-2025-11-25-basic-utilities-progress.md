+++
title = "specification-2025-11-25-basic-utilities-progress"
[extra]
skill = false
...
3. For [task-augmented requests](@/skills/engineering/mcp/resources/auto/specification-2025-11-25-basic-utilities-tasks.md), the `progressToken` provided in the original request **MUST** continue to be used for progress notifications throughout the task's lifetime, even after the `CreateTaskResult` has been returned. The progress token remains valid and associated with the task until the task reaches a terminal status.
