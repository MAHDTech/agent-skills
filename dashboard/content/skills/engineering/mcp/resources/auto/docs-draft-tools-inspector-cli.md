+++
title = "docs-draft-tools-inspector-cli"
[extra]
skill = false
...
See [Configuration and flags](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration) for `--catalog` vs. `--config`, the `--` separator, and the shared server-selection flags.
...
See [Authorization](https://modelcontextprotocol.io/docs/draft/tools/inspector/authorization) for the full flow, the web-to-CLI handoff, and `--print-handoff`.
