+++
title = "docs-draft-tools-inspector"
[extra]
skill = false
...
The [MCP Inspector](https://github.com/modelcontextprotocol/inspector) is the reference developer tool for testing and debugging [MCP servers](https://modelcontextprotocol.io/docs/draft/learn/server-concepts). It ships as a single package, `@modelcontextprotocol/inspector`, providing **three clients behind one binary**:
...
\<Tabs\>
  <Tab title="Web">
    ...
    The command prints a URL containing a one-time session token; open it in your browser. See [Web client](https://modelcontextprotocol.io/docs/draft/tools/inspector/web).
  </Tab>
  ...
  <Tab title="CLI">
    ...
    See [CLI client](https://modelcontextprotocol.io/docs/draft/tools/inspector/cli).
  </Tab>
  ...
  <Tab title="TUI">
    ...
    See [TUI client](https://modelcontextprotocol.io/docs/draft/tools/inspector/tui).
  </Tab>
</Tabs>
...
Everything else (`--catalog`, `--config`, `--server-url`, `--transport`, `--method`, the OAuth flags) is defined by the *client*, not the launcher, and the clients do not all define the same set. The [Configuration and flags](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration) page is organized that way, by owner.
