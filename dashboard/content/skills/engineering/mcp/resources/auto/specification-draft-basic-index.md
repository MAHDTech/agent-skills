+++
title = "specification-draft-basic-index"
[extra]
skill = false
...
[Requests](https://modelcontextprotocol.io/specification/draft/schema#jsonrpcrequest) are sent from the client to the server, to initiate an operation.
...
[Result responses](https://modelcontextprotocol.io/specification/draft/schema#jsonrpcresultresponse) are sent when the operation completes successfully.
...
* A `resultType` of `"input_required"` indicates the request is incomplete and more information is needed to process the request. The result contains an [`InputRequiredResult`](https://modelcontextprotocol.io/specification/draft/basic/patterns/mrtr#inputrequiredresult) object with additional information needed.
...
[Error responses](https://modelcontextprotocol.io/specification/draft/schema#jsonrpcerrorresponse) are sent when the operation fails or encounters an error.
...
* **`-32020` to `-32099` — reserved for the MCP specification.** Error codes
  ...
  recorded in the [schema](https://modelcontextprotocol.io/specification/draft/schema). Implementations
...
| Code     | Name                                                                                                  |
| -------- | ----------------------------------------------------------------------------------------------------- |
| `-32020` | [`HeaderMismatch`](https://modelcontextprotocol.io/specification/draft/schema#headermismatcherror)                                   |
| `-32021` | [`MissingRequiredClientCapability`](https://modelcontextprotocol.io/specification/draft/schema#missingrequiredclientcapabilityerror) |
| `-32022` | [`UnsupportedProtocolVersion`](https://modelcontextprotocol.io/specification/draft/schema#unsupportedprotocolversionerror)           |
...
* `-32002` — resource not found (2025-11-25 and earlier; replaced by `-32602`).
  ...
  accept `-32002`](https://modelcontextprotocol.io/specification/draft/server/resources#error-handling) from
...
[Notifications](https://modelcontextprotocol.io/specification/draft/schema#jsonrpcnotification) are sent from the client to the server or vice versa, as a one-way message.
...
The Model Context Protocol (MCP) supports several [Message Patterns](https://modelcontextprotocol.io/specification/draft/basic/patterns) that define how clients and servers interact:

1. **[Request and Response](https://modelcontextprotocol.io/specification/draft/basic/patterns#request-and-response)**: A client sends a request to the server, and the server responds with a result or error.
2. **[Multi Round-Trip Requests (MRTR)](https://modelcontextprotocol.io/specification/draft/basic/patterns#multi-round-trip-requests)**: A server requires additional client input (sampling, elicitation, or roots) to complete a request.
3. **[Subscribe and Notify](https://modelcontextprotocol.io/specification/draft/basic/patterns#subscribe-and-notify)**: A client subscribes to a stream of notifications from the server, which are sent as they occur.
...
[`subscriptions/listen`](https://modelcontextprotocol.io/specification/draft/basic/patterns/subscriptions)
...
\<Info\>
  ...
  [Architecture guide](https://modelcontextprotocol.io/docs/draft/learn/architecture#example).
</Info>
...
| -------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------ |
| `progressToken`                              | Opts the request into progress notifications                | [Progress](https://modelcontextprotocol.io/specification/draft/basic/patterns/progress)           |
...
| `io.modelcontextprotocol/logLevel`           | Minimum log level the server should emit for a request      | [Logging](https://modelcontextprotocol.io/specification/draft/server/utilities/logging)           |
| `io.modelcontextprotocol/subscriptionId`     | Correlates a notification with its originating subscription | [Subscriptions](https://modelcontextprotocol.io/specification/draft/basic/patterns/subscriptions) |
| `traceparent`, `tracestate`, `baggage`       | OpenTelemetry trace context propagation                     | OpenTelemetry trace context (below)                                |

Official [extensions](https://modelcontextprotocol.io/specification/draft/basic/versioning#extension-negotiation)
...
[`MissingRequiredClientCapabilityError`](https://modelcontextprotocol.io/specification/draft/schema#missingrequiredclientcapabilityerror)
