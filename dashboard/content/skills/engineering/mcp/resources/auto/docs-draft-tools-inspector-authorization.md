+++
title = "docs-draft-tools-inspector-authorization"
[extra]
skill = false
...
| The server's `oauth` block in the [catalog file](https://modelcontextprotocol.io/docs/draft/tools/inspector/configuration#catalog-file-format) | Per-server client id/secret, scopes, the enterprise-managed flag, and the [step-up](#mid-session-re-authorization) policy.             |
...
The `deepLink` in the handoff block navigates a browser straight to a *connected* Inspector; see [Deep links](https://modelcontextprotocol.io/docs/draft/tools/inspector/web#deep-links).
