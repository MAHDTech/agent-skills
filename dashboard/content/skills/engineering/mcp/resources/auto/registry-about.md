...
The [Package Types guide](@/skills/engineering/mcp/resources/auto/registry-package-types.md) lists the supported package types and registries. More package registries may be supported in the future based on community demand. If you are interested in building support for a package registry, please [open an issue](https://github.com/modelcontextprotocol/registry).
...
We expect that downstream aggregators will use the MCP Registry API to pull new metadata on a regular but infrequent basis (for example, once per hour). See the [MCP Registry Aggregators guide](@/skills/engineering/mcp/resources/auto/registry-registry-aggregators.md) for more information.
...
This namespace system ensures that only the legitimate owner of a GitHub account or domain can publish servers under that namespace, providing trust and accountability in the ecosystem. For details on authentication methods, see the [Authentication guide](@/skills/engineering/mcp/resources/auto/registry-authentication.md).
...
* **Manual takedown** — The registry maintainers can manually remove spam or malicious servers. See the [Moderation Policy](@/skills/engineering/mcp/resources/auto/registry-moderation-policy.md) for details on what content is removed.
