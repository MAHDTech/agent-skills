+++
title = "docs-draft-tools-inspector-configuration"
[extra]
skill = false
...
The whole scripting surface belongs to the CLI. See [CLI client](https://modelcontextprotocol.io/docs/draft/tools/inspector/cli) for usage.
...
| Variable                                  | Effect                                                                                                                    |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `MCP_INSPECTOR_API_TOKEN`                 | Pin the [session token](https://modelcontextprotocol.io/docs/draft/tools/inspector/web#the-session-token) instead of generating a random one per launch. |
...
Fields that equal their default are omitted when the Inspector writes the file back, keeping diffs minimal. `protocolEra` (see [Protocol eras](https://modelcontextprotocol.io/docs/draft/tools/inspector/protocol-eras)) defaults to `legacy` and `modernLogLevel` to `debug`.

You do not have to hand-write these; the web client can [import an existing client config](https://modelcontextprotocol.io/docs/draft/tools/inspector/recipes#importing-an-existing-client-config) from Claude Desktop, Cursor, Cline, or VS Code, or a registry `server.json`.
