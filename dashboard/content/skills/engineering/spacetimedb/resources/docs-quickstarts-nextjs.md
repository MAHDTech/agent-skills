...
SpacetimeDB module and Next.js client.

This will start the local SpacetimeDB server, publish your module,
generate TypeScript bindings, and start the Next.js development server.
...
to SpacetimeDB via environment variables in `.env.local`.

**Explore the project structure**

Your project contains both server and client code using the Next.js App
Router.

Edit `spacetimedb/src/index.ts` to add tables and reducers. Edit
`app/page.tsx` and `app/PersonList.tsx` to build your UI.
...
- **Server Component** (`page.tsx`): Fetches initial data during SSR for
  fast page loads
- **Client Component** (`PersonList.tsx`): Maintains a real-time
  WebSocket connection for live updates

The `lib/spacetimedb-server.ts` file provides a utility for server-side
data fetching.
...
In client components, use `useTable` to subscribe to table data and
`useReducer` to call reducers. The Server Component passes initial data
as props for instant rendering.
