...
A **procedure** is a function exported by a [database](/docs/databases),
similar to a [reducer](/docs/functions/reducers). Connected
[clients](/docs/clients) can call procedures. Procedures can perform
...
`ReducerCtx`](/docs/functions/reducers/reducer-context) When the
...
ways as a `ReducerContext`](/docs/functions/reducers/reducer-context).
...
`[](TxContext& tx) -> T`. Within that function, the `TxContext` can be
used to access the database [in all the same ways as a
`ReducerContext`](/docs/functions/reducers/reducer-context). When the
function returns, the transaction will be committed, and its changes to
the database state will become permanent and be broadcast to clients. If
...
`[](TxContext& tx) -> bool`. If the function returns `true`, the
transaction will be committed, and its changes to the database state
...
![Calling
Procedures](/docs/assets/images/ue-blueprint-calling-procedure-265d7e3c7cf419716bdd4d6fbdfd2489.png)
...
![Procedure
Callbacks](/docs/assets/images/ue-blueprint-procedure-callback-a022ea5b8cbc6a85706ac0375f781677.png)
