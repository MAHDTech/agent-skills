+++
title = "docs-tutorials-Godot-part-4"
[extra]
skill = false
...
3](https://spacetimedb.com/docs/tutorials/Godot/part-3).
...
the `Update` loop in a game engine like Godot. We've scheduled it every
...
- Update the URL in the Main node to:
...
So far you've learned how to configure a new Godot project to work with
...
You've also learned how to view module logs and connect your client to
your database server, call reducers from the client and synchronize the
data with client. Finally you learned how to use that synchronized data
to draw game objects on the screen, so that we can interact with them
and play a game!
...
column, so that does not prevent players to use the same username on the
same server.
