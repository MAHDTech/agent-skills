...
Follow these guidelines to optimize table performance in your
SpacetimeDB modules.
...
Add indexes to columns you frequently filter or join on. See
[Indexes](https://spacetimedb.com/docs/tables/indexes) for details.
...
Make tables public only when clients need to access them. Private
tables:
...
When inserting or updating multiple rows, batch them in a single reducer
call rather than making multiple reducer calls:
...
- Learn about [Indexes](https://spacetimedb.com/docs/tables/indexes) to optimize queries
