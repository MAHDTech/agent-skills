...
- [Reducer Callbacks](#reducer-callbacks)
  - [What changed](#what-changed)
  - [Option A: Per-call result callbacks
    (`_then()`)](#option-a-per-call-result-callbacks-_then)
  - [Option B: Event tables (recommended for most use
    cases)](#option-b-event-tables-recommended-for-most-use-cases)
  - [Why event tables are better](#why-event-tables-are-better)
  - [Event table details](#event-table-details)
- [Event Type Changes](#event-type-changes)
  - [What changed](#what-changed-1)
- [Subscription API](#subscription-api)
- [Table Name Canonicalization](#table-name-canonicalization)
  - [What changed](#what-changed-2)
  - [Updating source code: Change `name` to `accessor` in table
    definitions](#updating-source-code-change-name-to-accessor-in-table-definitions)
  - [Auto-migrating existing
    databases](#auto-migrating-existing-databases)
- [Clients connect with database
  name](#clients-connect-with-database-name)
- [`sender` Is Now A Method, Not A
  Field](#sender-is-now-a-method-not-a-field)
- [Only Primary Keys Have Update
  Methods](#only-primary-keys-have-update-methods)
  - [Updates which preserve the primary key - update with the primary
    key
    index](#updates-which-preserve-the-primary-key---update-with-the-primary-key-index)
  - [Updates which change the primary key - explicitly delete and
    insert](#updates-which-change-the-primary-key---explicitly-delete-and-insert)
  - [Updates which preserve the primary key - update with the primary
    key
    index](#updates-which-preserve-the-primary-key---update-with-the-primary-key-index-1)
  - [Updates which change the primary key - explicitly delete and
    insert](#updates-which-change-the-primary-key---explicitly-delete-and-insert-1)
  - [Updates which preserve the primary key - update with the primary
    key
    index](#updates-which-preserve-the-primary-key---update-with-the-primary-key-index-2)
  - [Updates which change the primary key - explicitly delete and
    insert](#updates-which-change-the-primary-key---explicitly-delete-and-insert-2)
- [Scheduled Functions Are Now
  Private](#scheduled-functions-are-now-private)
  - [Remove authorization logic from scheduled
    functions](#remove-authorization-logic-from-scheduled-functions)
  - [Define wrappers for functions that are both scheduled and invoked
    by
    clients](#define-wrappers-for-functions-that-are-both-scheduled-and-invoked-by-clients)
- [Private Items Are Not Code-Generated By
  Default](#private-items-are-not-code-generated-by-default)
  - [Pass `--include-private` to
    `spacetime generate`](#pass---include-private-to-spacetime-generate)
- [Light Mode](#light-mode)
  - [What changed](#what-changed-3)
- [CallReducerFlags](#callreducerflags)
  - [What changed](#what-changed-4)
- [Confirmed Reads Enabled by
  Default](#confirmed-reads-enabled-by-default)
