+++
title = "docs-quickstarts-browser"
[extra]
skill = false
...
The generated TypeScript bindings need to be bundled into a JavaScript
file that can be loaded in the browser via a script tag.
...
Open `index.html` directly in your browser. The app connects to
SpacetimeDB and displays data in real-time.

The JavaScript code runs inline in a script tag, using the bundled
`DbConnection` class.
...
Reducers are functions that modify data — they're the only way to write
to the database.
