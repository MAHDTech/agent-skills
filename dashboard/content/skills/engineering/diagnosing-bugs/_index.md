...
10. **HITL bash script.** Last resort. If a human must click, drive _them_ with `resources/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you.
...
- [ ] **Agent-runnable** — you can run it unattended; a human in the loop only via `resources/hitl-loop.template.sh`.
