+++
title = "build-features-sessionstodos"
[extra]
skill = false
...
`/fork [directive]` branches the current session into a peer that starts from a copy of the conversation. Pass `--worktree` or `--no-worktree` to choose whether the fork runs in an isolated copy of the repository, so parallel sessions do not overwrite each other's files — see [Worktrees](https://docs.x.ai/build/features/worktrees).
