+++
title = "developers-model-capabilities-imagine-files-inputs"
[extra]
skill = false
...
Anywhere an Imagine endpoint accepts a public URL or base64-encoded image/video, you can substitute a `file_id` from your [Files API](https://docs.x.ai/developers/files) storage. The file is fetched server-side from your private storage, so:
...
* Works with both uploaded files and assets generated by earlier Imagine calls (via [`storage_options`](https://docs.x.ai/developers/model-capabilities/imagine/files/outputs)).
