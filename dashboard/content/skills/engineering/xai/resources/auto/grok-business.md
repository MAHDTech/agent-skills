+++
title = "grok-business"
[extra]
skill = false
category = "engineering"
mermaid = false
skill_name = "xai"
+++

{% raw %}
[](https://x.ai/)

*   [Products](https://x.ai/grok)
*   [Solutions](https://x.ai/solutions)
*   [Developer](https://x.ai/api)
*   [Company](https://x.ai/company)
*   [Pricing](https://x.ai/pricing)
*   [News](https://x.ai/news)

[Contact Sales](https://x.ai/contact-sales)

[Try for free](https://grok.com/?referrer=website)

Products

[Grok](https://grok.com/?referrer=website)[Business](https://x.ai/grok/business)[Government](https://x.ai/grok/government)

Download

[iOS](https://apps.apple.com/app/apple-store/id6670324846)[Android](https://play.google.com/store/apps/details?id=ai.x.grok)[Grok on X](https://x.com/i/grok)

Developers

[API Console](https://console.x.ai/)[Documentation](https://docs.x.ai/)[CLI](https://x.ai/cli)

[Try for free](https://grok.com/?referrer=website)

SpaceXAI for Business

# Give your business

superpowers.

Frontier AI for your enterprise. Real-time search, voice, image, and video generation — all with enterprise-grade security and controls.

[Try Free](https://accounts.x.ai/sign-up?redirect=cloud-console&action=grok-business)Contact Sales

SOC 2 Type II HIPAA GDPR & CCPA SSO & SCIM

Summarize our recent order forms and put them into a pitch deck

I've compiled 3 documents from your recent orders:

Order_Summary.xlsx

Aggregated order data — 142 line items

Supplier_Terms.docx

Vendor terms extracted from 6 contracts

Pitch_Deck.pptx

Executive summary — 12 slides

Ask Grok anything...

Fast

Features

## Enterprise-grade AI, ready to deploy

Secure data isolation, real-time knowledge across your tools, and production-ready outputs for every team.

### Secure, Enterprise Vault

Your data is encrypted with your keys, under your control, and isolated from all other customers.

### Real-time search and knowledge

Connect your external applications and allow SpaceXAI to seamlessly search across conversations, documents and emails.

### Slides, spreadsheets, and documents

You're not confined to a chat feed; Grok can produce a variety of outputs.

forecast_model.xlsx

A

B

C

D

1

Revenue

$1.2M

$1.5M

$1.8M$2.02M

2

COGS

$520K

$610K

$720K

3

OpEx

$180K

$210K

$240K

4

Gross profit

$680K

$890K

$1.08M$1.30M

5

Margin

56.7%

59.3%

60.0%64.2%

Increase Q4 revenue by 12% and update margins

### Connect your tools

Use Connectors to integrate your tools, automate workflows, and make your workplace more efficient.

*   Salesforce, HubSpot, Slack, Notion, and more
*   Read and update records end-to-end
*   Bring your own MCP server

Build an account plan for Enterprise prospects in Q4 pipeline

Here's a Q4 plan covering 12 enterprise targets, ranked by deal size and buying signals. 

Research competitor pricing and create sales battlecards

Analyzed 5 competitors. Key differentiators: real-time data, multi-model access, and 40% lower TCO. 

Draft personalized outreach sequences for decision-makers

Created 3 outreach sequences tailored to VP, Director, and C-suite personas with personalized hooks. 

What do you want to know?

Fast

### Turn ideas into specs

Turn rough ideas into structured technical documents. Grok helps outline system designs, API contracts, and architecture decisions so your team can align faster.

*   API references, runbooks, and ADRs
*   Reads source files to infer architecture
*   Reviews docs and flags stale sections

Engineering/Design Docs/Auth Service v2

# Auth Service Redesign

Status Draft In Review

Owner S. Chen

Updated Just now

## Overview

Migrate from session-based authentication to a stateless JWT architecture with refresh token rotation and PKCE support for public clients.

## Architecture

*   •API Gateway validates tokens at the edge
*   •Access tokens: 15min TTL, RS256 signed
*   •Refresh tokens stored in encrypted HttpOnly cookies
*   •Key rotation via /.well-known/jwks.json

## API Contract

POST /v2/auth/token

Draft a design doc for the auth service redesign

Fast

### Channel-aware copywriting

Draft campaign copy, subject lines, and landing page content that matches your brand voice. Iterate in seconds instead of hours.

*   Channel-aware format and length
*   Leverages real-time knowledge from X
*   Localization across markets, not just translation

Create customer marketing campaigns using agent builder

Built a 3-channel campaign targeting mid-market accounts with personalized messaging sequences. 

Pull real-time user sentiment analysis to create copy

Sentiment is 72% positive. Key themes: speed and reliability. Here's copy that leans into both. 

Generate A/B test variants for email subject lines

8 variants across urgency, curiosity, and social proof angles. Predicted top performer: 34% open rate. 

What do you want to know?

Fast

### Voice agents

Deploy AI voice agents that handle routine customer questions around the clock. Reduce wait times and free your team to focus on complex issues.

*   Sub-second latency over WebSockets
*   25+ expressive voices plus custom voice cloning
*   Graceful handoff to human agents

What do you want to know?

Fast

### Smarter spreadsheets

Build and audit spreadsheets with accurate formulas, clean formatting, and automated data analysis. Get from raw numbers to actionable insights faster.

*   Excel files with formulas and dashboards
*   Edit Google Sheets and Excel via connectors
*   Period-over-period and peer benchmarking

Q1 '25

Q2 '25

Q3 '25

Q4 '25

FY '25

YoY

1

Revenue

$4.2M

$4.8M

$5.1M

$5.6M$6.1M

$19.7M$20.2M

+18%+21%

2

COGS

$1.7M

$1.9M

$2.0M

$2.2M

$7.8M

—

3

Gross Prof.

$2.5M

$2.9M

$3.1M

$3.4M$3.9M

$11.9M$12.4M

+28%+34%

4

OpEx

$1.4M

$1.5M

$1.6M

$1.7M

$6.2M

—

5

EBITDA

$1.8M

$2.1M

$2.3M

$2.6M$2.9M

$8.8M$9.2M

+22%+28%

6

Net Inc.

$1.1M

$1.4M

$1.5M

$1.7M$2.2M

$5.7M$6.2M

+24%+31%

7

Burn Mult.

0.9x

0.8x

0.7x

0.6x 0.5x

0.75x 0.63x

—

P&L Summary

Raw Data

Charts

Revise Q4 projections with 15% higher bookings and recalculate YTD totals

Fast

Build an account plan for Enterprise prospects in Q4 pipeline

Here's a Q4 plan covering 12 enterprise targets, ranked by deal size and buying signals. 

Research competitor pricing and create sales battlecards

Analyzed 5 competitors. Key differentiators: real-time data, multi-model access, and 40% lower TCO. 

Draft personalized outreach sequences for decision-makers

Created 3 outreach sequences tailored to VP, Director, and C-suite personas with personalized hooks. 

What do you want to know?

Fast

## Pricing

### Business

[Create a Team](https://accounts.x.ai/sign-up?redirect=cloud-console&action=grok-business)

Perfect for small-to-medium teams collaborating with Grok to innovate.

$30/ month per user

[Create a Team](https://accounts.x.ai/sign-up?redirect=cloud-console&action=grok-business)

### Enterprise

[Contact Sales](https://x.ai/grok/business#contact-form)

Ideal for large organizations using advanced features and custom solutions.

Let's talk

[Contact Sales](https://x.ai/grok/business#contact-form)

### Models

Imagine

Voice

Grok 4.5

### Security & compliance

SOC 2 compliance (Type I & II)

Role Based Access Control

Increased rate limits

No training

Team + seat management

Consolidated billing

Domain Verification and Claims

User analytics

Custom data retention

Connectors

Advanced audit and security controls

Grok Build

Custom Single Sign On (SSO)

Directory Sync (SCIM)

Custom Role Based Access Control

Advanced User & Access Management

Dedicated onboarding and ongoing support

Customer-Managed Encryption Keys

Application-Level Encryption

Dedicated Data Plane

## Pricing

### Business

[Create a Team](https://accounts.x.ai/sign-up?redirect=cloud-console&action=grok-business)

Perfect for small-to-medium teams collaborating with Grok to innovate.

$30/ month per user

[Create a Team](https://accounts.x.ai/sign-up?redirect=cloud-console&action=grok-business)

### Models

Imagine

Voice

Grok 4.5

### Security & compliance

SOC 2 compliance (Type I & II)

Role Based Access Control

Increased rate limits

No training

Team + seat management

Consolidated billing

Domain Verification and Claims

User analytics

Custom data retention

Connectors

Advanced audit and security controls

Grok Build

### Enterprise

[Contact Sales](https://x.ai/grok/business#contact-form)

Ideal for large organizations using advanced features and custom solutions.

Let's talk

[Contact Sales](https://x.ai/grok/business#contact-form)

### Models

Imagine

Voice

Grok 4.5

### Security & compliance

SOC 2 compliance (Type I & II)

Role Based Access Control

Increased rate limits

No training

Team + seat management

Consolidated billing

Domain Verification and Claims

User analytics

Custom data retention

Connectors

Advanced audit and security controls

Grok Build

Custom Single Sign On (SSO)

Directory Sync (SCIM)

Custom Role Based Access Control

Advanced User & Access Management

Dedicated onboarding and ongoing support

Customer-Managed Encryption Keys

Application-Level Encryption

Dedicated Data Plane

## Let's talk

Interested in bringing SpaceXAI to your organization? Fill out the form and a member of our team will be in touch shortly.

First Name * 

Last Name * 

Email Address * 

Company Name * 

Company Size * 

Select size

Job Title/Role * 

Number of Enterprise Licenses * 

Monthly 

Annual 

Enterprise Needs * 

Request Access

© 2026 X.AI LLC

[Built with Grok](https://grok.com/?referrer=website)

Products

[Chat](https://x.ai/grok)[Build](https://x.ai/cli)[Imagine](https://x.ai/api/imagine)[Voice](https://x.ai/voice)[Grokipedia](https://grokipedia.com/)

Download

[grok.com](https://grok.com/?referrer=website)[iOS](https://apps.apple.com/app/apple-store/id6670324846)[Android](https://play.google.com/store/apps/details?id=ai.x.grok)[Grok on X](https://x.com/i/grok)

Solutions

[Business](https://x.ai/grok/business)[Government](https://x.ai/grok/government)[Customer Support](https://x.ai/solutions/customer-support)[Legal](https://x.ai/solutions/legal)[Security](https://x.ai/solutions/security)[Use Cases](https://x.ai/grok/use-cases)

Developers

[API Overview](https://x.ai/api)[Pricing](https://x.ai/pricing)[Models](https://docs.x.ai/developers/models)[Console](https://console.x.ai/)[Docs](https://docs.x.ai/)[Status](https://status.x.ai/)

Enterprise

[Contact Sales](https://x.ai/contact-sales)[FAQs](https://x.ai/legal/faq-enterprise)[BAA](https://x.ai/legal/baa)[DPA](https://x.ai/legal/data-processing-addendum)

Company

[About](https://x.ai/company)[Colossus](https://x.ai/colossus)[Careers](https://x.ai/careers)[News](https://x.ai/news)[Contact](https://x.ai/contact)

Trust

[Safety](https://x.ai/safety)[Security](https://x.ai/security)[Privacy Portal](https://x.ai/privacy-portal)[Subprocessors](https://x.ai/legal/subprocessor-list)[Help Center](https://docs.x.ai/grok/user-guide)

[Legal](https://x.ai/legal)

[Terms](https://x.ai/legal/terms-of-service)[Enterprise Terms](https://x.ai/legal/terms-of-service-enterprise)[Privacy](https://x.ai/legal/privacy-policy)[Cookies](https://x.ai/legal/cookie-policy)[AUP](https://x.ai/legal/acceptable-use-policy)[Brand](https://x.ai/legal/brand-guidelines)

Social

[@spacexai](https://x.com/spacexai)[@grok](https://x.com/grok)[Discord](https://discord.com/invite/kqCc86jM55)

[Built with Grok](https://grok.com/?referrer=website)

Copy black SVG

Copy white SVG
{% endraw %}
