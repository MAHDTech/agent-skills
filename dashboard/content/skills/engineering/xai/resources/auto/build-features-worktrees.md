+++
title = "build-features-worktrees"
[extra]
skill = false
...
In the TUI: `/fork --worktree` forks the current session into a worktree, `Ctrl+W` on the welcome screen opens the New Worktree dialog, and `Ctrl+W` in the [Agent Dashboard](https://docs.x.ai/build/features/dashboard) dispatches new agents into worktrees. Whether `/new` and `/fork` offer a worktree is configurable — see [TOML Values](https://docs.x.ai/build/settings/reference#toml-values).
