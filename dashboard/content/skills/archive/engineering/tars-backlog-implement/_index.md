...
source_url = "https://github.com/MAHDTech/agent-skills/tree/main/skills-archive/engineering/tars-backlog-implement"
+++
...
- **If `.tars/run.env` does not exist, do not proceed and do not improvise.** Read [tars-backlog-prepare](@/skills/archive/planning/tars-backlog-prepare/_index.md) and execute its steps inline, then continue. See **Invoking Sibling Skills** below for why you cannot simply call it.
...
All backlog operations run from a topic branch (never the default branch). Every spoke branches off this active topic branch, and all approved changes are merged back into it. See the canonical **Topic Branch Verification** section in [tars-backlog-prepare](@/skills/archive/planning/tars-backlog-prepare/_index.md) for the full policy and commands.
...
**Devenv:** if the project has `devenv.nix` / `devenv.yaml`, prepare already followed the [devenv](@/skills/skills/tooling/devenv/_index.md) skill when freezing commands. Implement does not restate secrets or CI env vars - it only runs `tars-gate` / `tars-spoke`.
...
6. **Implementation review** (risk-tiered), only on green clean tree:
   ...
   | **Full** [tars-backlog-review](@/skills/archive/review/tars-backlog-review/_index.md) | Any of: `risk: high`; diff touches a path matching `TARS_REVIEW_ALWAYS_FULL` globs from `run.env`; **post-conflict** resolve this ticket; `attempts >= 2` / rework; Hub marks security/shared-core | Dual-axis Spec + Standards                                                                                  |
...
- [tars-backlog-prepare](@/skills/archive/planning/tars-backlog-prepare/_index.md) - integrity, spoke root, command freeze, baseline smoke, `run.env`.
- [tars-backlog-review](@/skills/archive/review/tars-backlog-review/_index.md) - full dual-axis review when risk-tiered rules require it.
- [tars-backlog-create-issue](@/skills/archive/planning/tars-backlog-create-issue/_index.md) - ticket format including `files:` / `owns:`.
- [devenv](@/skills/skills/tooling/devenv/_index.md) - only if you must understand how prepare built commands; implement does not re-enter devenv by hand when `run.env` is present.
