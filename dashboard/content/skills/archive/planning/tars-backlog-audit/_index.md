...
source_url = "https://github.com/MAHDTech/agent-skills/tree/main/skills-archive/planning/tars-backlog-audit"
+++
...
5. **Generate Tickets**: For each verified finding, write a new ticket file to `.tars/issues/todo/` following the guidelines and structure defined in the [tars-backlog-create-issue](@/skills/archive/planning/tars-backlog-create-issue/_index.md) skill.
   ...
   - **Frontmatter shapes are load-bearing**: `dependencies:` must be an inline array on one line, `files:` a two-space-indented block list. See **Frontmatter field rules** in [tars-backlog-create-issue](@/skills/archive/planning/tars-backlog-create-issue/_index.md); getting these wrong is not a formatting nit, it silently drops the data.
   - The batching rules that consume `files:`, `owns:`, and `dependencies:` are stated canonically in [tars-backlog-implement](@/skills/archive/engineering/tars-backlog-implement/_index.md). Do not restate them in a ticket.
...
7. **CRITICAL CLEANLINESS ASSERTION**: Audit agents read the parent working tree in place, so the Hub must confirm they left it untouched - run this after every batch, whether the agents succeeded, failed, or timed out:
   ...
   This is safe here because spoke workspaces live outside the repository tree entirely (see `TARS_SPOKE_ROOT` in [tars-backlog-prepare](@/skills/archive/planning/tars-backlog-prepare/_index.md)), so no in-flight work can be caught by it. There are no worktrees or branches to tear down.
