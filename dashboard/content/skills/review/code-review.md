...
On top of whatever the repo documents, the Standards axis always carries a **smell baseline** — a fixed set of common code smells that applies even when a repo documents nothing. It lives in [code-smells.md](code-smells.md). Two rules bind it:
...
- The list of standards-source files you found in step 3, **plus the full contents of [code-smells.md](code-smells.md) pasted in** — the sub-agent has no other access to that baseline.
