...
This matters when a ticket is corrected: a note added to `## Review` saying a value is stale does **not** change what gets implemented. The stale value inside the binding section must itself be edited. See the repair duty in [tars-backlog-triage](@/skills/planning/tars-backlog-triage/_index.md).

## Batch and dependency rules

Do not restate the batching rules in a ticket. They are stated once, canonically, in the **Conflict-Free Batching** section of [tars-backlog-implement](@/skills/engineering/tars-backlog-implement/_index.md); a ticket that duplicates them will disagree with them the first time they change.
