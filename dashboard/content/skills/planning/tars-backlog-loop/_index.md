...
3. As each spoke reports, the Hub fetches its branch into the parent for durability, then runs the full verification gate — `prek run -a` plus the test suite (using the [prek](@/skills/tooling/prek/_index.md) tool) — **inside the spoke's clone, under the mutex, before merging anything**. A red gate goes straight back to the live spoke to fix, for up to 3 rounds.
4. On a green gate, the Hub calls `tars-backlog-review` (see [tars-backlog-review](@/skills/review/tars-backlog-review/_index.md)) for the double-axis verdict. Approved branches merge sequentially into the topic branch and their tickets move to `.tars/issues/done/`.
