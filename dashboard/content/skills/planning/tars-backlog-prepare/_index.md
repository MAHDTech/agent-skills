...
**Suggested checks** for a repo-supplied linter - the batching invariants from the canonical rules in [tars-backlog-implement](@/skills/engineering/tars-backlog-implement/_index.md) that are mechanically checkable and exactly what a pre-dispatch check exists to catch:
...
>
> Every frozen string that enters a devenv shell (`TARS_INSTALL_COMMAND`, `TARS_HOOK_COMMAND`, `TARS_TEST_COMMAND`, and `TARS_CI_COMMAND` if it enters one) **must** carry, per the [devenv](@/skills/tooling/devenv/_index.md) skill's non-interactive shape:
>
> ```bash
