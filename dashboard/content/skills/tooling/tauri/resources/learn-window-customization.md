+++
title = "learn-window-customization"
[extra]
skill = false
category = "tooling"
mermaid = false
...
Tauri provides lots of options for customizing the look and feel of your
app’s window. You can create custom titlebars, have transparent windows,
enforce size constraints, and more.

## Configuration

There are three ways to change the window configuration:

- [Through tauri.conf.json](https://v2.tauri.app/reference/config/#windowconfig)
- [Through the JavaScript
  API](https://v2.tauri.app/reference/javascript/api/namespacewindow/#window)
...
- [(macOS) Transparent Titlebar with Custom Window Background
  Color](#macos-transparent-titlebar-with-custom-window-background-color)
...
```
{  "$schema": "../gen/schemas/desktop-schema.json",  "identifier": "main-capability",  "description": "Capability for the main window",  "windows": ["main"],  "permissions": [    "core:window:default",    "core:window:allow-close",    "core:window:allow-minimize",    "core:window:allow-toggle-maximize",    "core:window:allow-start-dragging"  ]}
```
...
Note that if you are using a Rust-based frontend, you can copy the code
above into a `<script>` element in your `index.html` file.

### Manual Implementation of `data-tauri-drag-region`

For use cases where you customize the drag behavior, you can manually
add an event listener with `window.startDragging` instead of using
`data-tauri-drag-region`.

#### HTML

From the code in the previous section, we remove
`data-tauri-drag-region` and add an `id`:
...
We are going to create the main window and change its background color
from the Rust side.
...
Add `cocoa` crate to dependencies so that we can use it to call the
macOS native API:

```
[target."cfg(target_os = \"macos\")".dependencies]objc2-app-kit = { version = "0.3.2", features = ["NSColor", "NSWindow", "objc2-core-foundation"] }
```
