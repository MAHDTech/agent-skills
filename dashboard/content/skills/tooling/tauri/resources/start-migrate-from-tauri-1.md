...
reference](/reference/cli/#migrate)
...
- `tauri > allowlist > protocol > assetScope` moved to
  ...
  scope](/security/asset-protocol/) for `enable`, glob patterns,
...
- `bundle > createUpdaterArtifacts` added, must be set when using the
  ...
  - set it to `v1Compatible` when upgrading from v1 apps that were
    already distributed. See the [updater guide](/plugin/updater/) for
    more information.
...
[Embedding External Binaries](/develop/sidecar/#running-it-from-rust)
guide to see how.
