...
We recommend reading the general [sidecar guide](/develop/sidecar/)
...
4.  ##### Setup plugin-shell permission
    ...
    After installing the [shell plugin](/plugin/shell/) make sure you
    configure the required capabilities.

    Note that we use `"args": true` but you can optionally provide an
    array `["hello"]`, [read more](/develop/sidecar/#passing-arguments).
...
5.  ##### Configure the Sidecar in the Tauri Application
    ...
    [`bundle > externalBin`](/reference/config/#externalbin) array:
