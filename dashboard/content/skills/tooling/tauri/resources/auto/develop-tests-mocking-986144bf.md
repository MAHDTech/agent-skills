+++
title = "develop-tests-mocking-986144bf"
[extra]
skill = false
category = "tooling"
mermaid = false
...
simulate windows or intercept IPC calls is common, so-called *mocking*.
The [`@tauri-apps/api/mocks`](https://v2.tauri.app/reference/javascript/api/namespacemocks/)
...
find more about the specific API in detail
[here](https://v2.tauri.app/reference/javascript/api/namespacemocks/#mockipc).
...
Sometimes you want to track more information about an IPC call; how many
times was the command invoked? Was it invoked at all? You can use
[`mockIPC()`](https://v2.tauri.app/reference/javascript/api/namespacemocks/#mockipc) with
...
To mock IPC requests to a sidecar or shell command you need to grab the
ID of the event handler when `spawn()` or `execute()` is called and use
this ID to emit events the backend would send back:
...
[Since `2.7.0`](https://v2.tauri.app/release/tauri/v2.7.0/)

There is partial support of the [Event
System](https://v2.tauri.app/develop/calling-frontend/#event-system) to simulate events
emitted by your Rust code via the `shouldMockEvents` option:
...
[`mockWindows()`](https://v2.tauri.app/reference/javascript/api/namespacemocks/#mockwindows)
method to create fake window labels. The first string identifies the
“current” window (i.e., the window your JavaScript believes itself in),
and all other strings are treated as additional windows.
