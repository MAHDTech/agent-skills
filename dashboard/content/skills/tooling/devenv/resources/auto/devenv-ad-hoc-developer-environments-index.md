+++
title = "devenv-ad-hoc-developer-environments-index"
[extra]
skill = false
category = "tooling"
mermaid = false
...
List types like `:pkgs` append to existing values by default. To replace the entire list instead, add a `!` suffix to the type:
...
1. One-off tasks that need specific dependencies
1. Trying different configurations before committing to a `devenv.nix` file
1. Creating lightweight environments without project setup
1. Creating a matrix of different options, e.g. testing different versions of Python
