+++
title = "secretspec-concepts-audit-723641c2"
[extra]
skill = false
category = "tooling"
mermaid = false
...
By default the log is written to the per-user state directory, one entry
per line in [JSON Lines](https://jsonlines.org/) format:
...
(secretspec follows the XDG state-directory convention on macOS too,
matching where it keeps its config, so the path is the same as on Linux.
Set `[audit] path` to override it.)

The file is created with owner-only permissions (`0600` on Unix), inside
an owner-only directory (`0700`). The first time secretspec writes to
it, it prints a one-time note telling you where the log is and how to
turn it off.
...
policy: when that policy applies, SecretSpec requires the caller to
...
The [`secretspec audit`](https://secretspec.dev/reference/cli/#audit) command reads it for you
...
The log is a single file capped at **1 MiB** by default. When it reaches
the cap it is truncated and started fresh, so disk usage stays bounded
without any log rotation to manage. This makes the log a size-bounded
recent record rather than a complete, permanent history — it is not
intended to satisfy long-term compliance retention on its own. Forward
it to a central system if you need that.

## Reliability

Auditing never blocks secret access. If the log cannot be written (for
example, a read-only filesystem), secretspec prints a `warning:` to
stderr and continues — your `get`, `set`, and `run` still work.

## Configuration

Auditing is a per-machine concern, so it is configured in your
**user-global config** (`~/.config/secretspec/config.toml`) under the
top-level `[audit]` table — not in the project’s `secretspec.toml`. This
means a repository you clone cannot turn off or redirect your audit log.
See the [configuration
reference](https://secretspec.dev/reference/configuration/#audit-logging) for all options. To
turn it off:
