...
- **Ruby** (a native C extension) statically links the `secretspec-ffi`
  C ABI at build time; **Go** (purego) loads it at runtime with no cgo.
...
- **Python** builds the resolver into a pyo3 extension shipped as a
  `cp39-abi3` wheel, and **Ruby** statically links the `secretspec-ffi`
