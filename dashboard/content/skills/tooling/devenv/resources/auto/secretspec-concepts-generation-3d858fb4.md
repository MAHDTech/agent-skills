+++
title = "secretspec-concepts-generation-3d858fb4"
[extra]
skill = false
category = "tooling"
mermaid = false
...
when missing. This is useful for passwords, tokens, and keys that do not
need to be shared across developers.

## Basic Usage

```
[profiles.default]DB_PASSWORD = { description = "Database password", type = "password", generate = true }API_TOKEN = { description = "API token", type = "hex", generate = { bytes = 32 } }SESSION_KEY = { description = "Session key", type = "base64", generate = { bytes = 64 } }REQUEST_ID = { description = "Request ID prefix", type = "uuid", generate = true }
```
...
The `command` type runs a shell command and uses its stdout as the
generated value:
...
- Subsequent runs find the stored value and skip generation
  (idempotent).
- The `null` provider (0.19+) instead returns a fresh generated value
  for only the current resolution.
...
reports describe the value as generated without minting it. Use a
writable provider instead when another process or later invocation must
retrieve the same value.

## Example

```
[profiles.default]# Auto-generated on first run, reused after thatDB_PASSWORD = { description = "Database password", type = "password", generate = true }
