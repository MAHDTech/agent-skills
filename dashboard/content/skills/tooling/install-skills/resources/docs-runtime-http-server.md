+++
title = "docs-runtime-http-server"
[extra]
skill = false
category = "tooling"
mermaid = false
...
HTML imports don't just serve HTML: they run Bun's [bundler](https://bun.com/bundler), JavaScript transpiler, and CSS parser, so you can build frontends with React, TypeScript, and Tailwind CSS.

For a complete guide to building full-stack applications with HTML imports, see [fullstack dev server](https://bun.com/bundler/fullstack).
...
`Bun.serve` can also listen for HTTP/3 over QUIC. Set `http3: true` together with [`tls`](@/skills/tooling/install-skills/resources/docs-runtime-http-tls.md); HTTP/3 requires TLS.
