+++
title = "docs-guides-test-migrate-from-jest"
[extra]
skill = false
category = "tooling"
mermaid = false
...
Bun implements most of Jest's matchers, but compatibility isn't 100%. See the compatibility table in [Writing tests](https://bun.com/test/writing-tests#matchers).

***

If you use `testEnvironment: "jsdom"` to run your tests in a browser-like environment, follow the [DOM testing with Bun and happy-dom](https://bun.com/guides/test/happy-dom) guide to inject browser APIs into the global scope. That guide uses [`happy-dom`](https://github.com/capricorn86/happy-dom), a leaner and faster alternative to [`jsdom`](https://github.com/jsdom/jsdom).
...
* `transform` — Bun supports TypeScript & JSX. Configure other file types with [plugins](https://bun.com/runtime/plugins).
...
* `verbose` — set `logLevel: "debug"` in [`bunfig.toml`](https://bun.com/runtime/bunfig#loglevel)
...
* [Mark a test as a todo](https://bun.com/guides/test/todo-tests)
* [Writing tests](https://bun.com/test/writing-tests)
