...
You can override this with the `debug.editor` setting in your [`bunfig.toml`](https://bun.com/runtime/bunfig).
...
Resolves a file path or module specifier using Bun's internal [module resolution](https://bun.com/runtime/module-resolution) algorithm. The first argument is the path to resolve, and the second argument is the "root". If no match is found, it throws an `Error`.
