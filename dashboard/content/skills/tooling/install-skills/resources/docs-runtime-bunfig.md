...
Configure how Bun maps file extensions to [loaders](https://bun.com/bundler/loaders). Use this to load file types Bun doesn't support natively.
...
When `true`, Bun does not run any `preinstall` / `install` / `postinstall` / `prepare` scripts, both for your project and for packages in `trustedDependencies`. See [Lifecycle scripts](https://bun.com/pm/lifecycle).
...
Configure Bun's [auto-install](https://bun.com/runtime/auto-install) behavior. Default `"auto"` — when no `node_modules` folder is found, Bun installs dependencies on the fly during execution.
...
See [Isolated installs](https://bun.com/pm/isolated-installs).
...
See [Global virtual store](https://bun.com/pm/global-store).
...
Learn more about [using and writing security scanners](https://bun.com/pm/security-scanner-api).
...
See [Minimum release age](https://bun.com/pm/cli/install#minimum-release-age).
