...
<Note>
  **Glob support** — Bun supports full glob syntax in `"workspaces"`, including negative patterns such as
  `!**/excluded/**`. See [supported glob patterns](https://bun.com/runtime/glob#supported-glob-patterns).
</Note>
...
* **Run scripts in multiple packages.** Use the [`--filter` flag](https://bun.com/pm/filter) to run `package.json` scripts in several packages at once, or `--workspaces` to run scripts across all workspaces.
...
references it. See [Catalogs](https://bun.com/pm/catalogs).
