...
<Warning>
  ...
  stable way to interact with native code from Bun is to write a [Node-API module](https://bun.com/runtime/node-api).
</Warning>
...
Thread-safe callbacks work best when run from another thread that is running JavaScript code, that is, a [`Worker`](https://bun.com/runtime/workers). A future version of Bun will enable them to be called from any thread, such as new threads spawned by your native library that Bun is not aware of.
