...
Bun supports TypeScript and JSX with no configuration. Bun transpiles every file on the fly with its native [transpiler](https://bun.com/runtime/transpiler) before running it.
...
Bun executes the script command in a subshell. On Linux & macOS, it checks for the following shells in order, using the first one it finds: `bash`, `sh`, `zsh`. On Windows, it uses the [Bun Shell](https://bun.com/runtime/shell) to support bash-like syntax and many common commands.
...
See [`--filter`](https://bun.com/pm/filter#running-scripts-with-filter).
