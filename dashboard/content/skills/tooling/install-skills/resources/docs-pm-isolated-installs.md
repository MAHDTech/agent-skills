+++
title = "docs-pm-isolated-installs"
[extra]
skill = false
category = "tooling"
mermaid = false
...
When [`install.globalStore`](https://bun.com/runtime/bunfig#install-globalstore) is enabled, store entries are materialized once into a [global virtual store](https://bun.com/pm/global-store) at `<cache>/links/` and `node_modules/.bun/<pkg>@<ver>` is a symlink into it. Warm installs after `rm -rf node_modules` only create one symlink per package instead of copying every package's files again, which is roughly **7× faster** on a typical mid-size project. The global store is **off by default**; see the [global store docs](https://bun.com/pm/global-store) for how to enable it, the full layout, benchmarks, and tradeoffs.
...
* [Package manager > Workspaces](https://bun.com/pm/workspaces) — Monorepo workspace management
* [Package manager > Lockfile](https://bun.com/pm/lockfile) — Understanding Bun's lockfile format
* [CLI > install](https://bun.com/pm/cli/install) — Complete `bun install` command reference
