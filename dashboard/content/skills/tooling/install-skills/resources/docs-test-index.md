...
Tests are written in JavaScript or TypeScript with a Jest-like API. See [Writing tests](https://bun.com/test/writing-tests).
...
The test runner runs all tests in a single process. It loads all `--preload` scripts (see [Lifecycle](https://bun.com/test/lifecycle)), then runs all tests. If a test fails, the test runner exits with a non-zero exit code.
...
See [Lifecycle](https://bun.com/test/lifecycle).
...
See [Mocks](https://bun.com/test/mocks).
...
See [Snapshots](https://bun.com/test/snapshots).
...
See [DOM testing](https://bun.com/test/dom).
