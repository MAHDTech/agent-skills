+++
title = "docs-guides-write-file-cat"
[extra]
skill = false
category = "tooling"
mermaid = false
...
Bun exposes `stdout` as a `BunFile` with the `Bun.stdout` property. Pass it as the destination to [`Bun.write()`](https://bun.com/runtime/file-io#writing-files-bun-write).
