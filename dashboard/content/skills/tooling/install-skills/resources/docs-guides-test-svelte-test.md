+++
title = "docs-guides-test-svelte-test"
[extra]
skill = false
category = "tooling"
mermaid = false
...
Use Bun's [Plugin API](https://bun.com/runtime/plugins) to add a custom loader for `.svelte` files, and the `test.preload` option in `bunfig.toml` to load it before your tests run.
