...
* **Node.js compatibility**. In addition to supporting Node-style module resolution, Bun aims for full compatibility with built-in Node.js globals (`process`, `Buffer`) and modules (`path`, `fs`, `http`). *This is an ongoing effort.* See [Node.js compatibility](https://bun.com/runtime/nodejs-compat) for the current status.
