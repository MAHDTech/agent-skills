...
The global virtual store is **off by default**. It only applies to the [isolated linker](https://bun.com/pm/isolated-installs); it is not used by the hoisted linker.
...
The on-disk layout adds one level of indirection compared to [isolated installs](https://bun.com/pm/isolated-installs#directory-structure):
...
[`publicHoistPattern`](https://bun.com/runtime/bunfig#install-publichoistpattern) and [`hoistPattern`](https://bun.com/runtime/bunfig#install-hoistpattern) hoist into the project's `node_modules`, which packages inside the global store can't reach. They still work for resolving hoisted packages from your own source code.
...
* [Package manager > Isolated installs](https://bun.com/pm/isolated-installs) — The linker the global store builds on
* [Package manager > Global cache](https://bun.com/pm/global-cache) — Where downloaded packages are stored
* [Runtime > bunfig](https://bun.com/runtime/bunfig#install-globalstore) — `bunfig.toml` reference
