...
CI verifies this by checking that the agent returns an `authMethods` array in its `initialize` response, with at least one method. See the [ACP auth methods RFD](@/skills/tooling/opencode-acp/resources/rfds-auth-methods.md) for the full specification.
