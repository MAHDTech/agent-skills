...
The agent will respond once the prompt has been *accepted*, not when the turn is over. Message IDs do not need to be returned from the prompt response; per the [message id RFD](/rfds/message-id), the agent remains the source of truth for message IDs and provides them through the session update stream when it emits the accepted user message.
