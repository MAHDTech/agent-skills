+++
title = "protocol-v1-authentication"
[extra]
skill = false
category = "tooling"
mermaid = true
...
ACP authentication is negotiated during [initialization](https://agentclientprotocol.com/protocol/v1/initialization). Agents advertise available authentication methods in `authMethods`, Clients choose one by calling `authenticate`, and Agents that support ending an authenticated state advertise the `logout` capability.
...
```json highlight={7-11,12-18} theme={null}
{
  "jsonrpc": "2.0",
  "id": 0,
  "result": {
...
See the [schema](https://agentclientprotocol.com/protocol/v1/schema#authmethod) for the full stable `AuthMethod` definition.
