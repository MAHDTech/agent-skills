+++
title = "rfds-introduce-rfd-process"
[extra]
skill = false
category = "tooling"
mermaid = false
...
#### Moving to the "active" section

When core maintainers or a working group are actively spending bandwidth on an RFD, the champion can open a PR to move the file to the active section. Active RFDs are the proposals the project is currently trying to move forward. This is a visibility signal, not a stability commitment or final approval.
...
* ✅ Establish lifecycle: Draft → Active → Preview → Completed
