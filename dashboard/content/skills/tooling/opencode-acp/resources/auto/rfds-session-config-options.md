+++
title = "rfds-session-config-options"
[extra]
skill = false
category = "tooling"
mermaid = false
...
Currently, we allow Agents to [specify a list of modes](https://agentclientprotocol.com/protocol/v1/session-modes) they can run in. The state of the currently selected item is allowed to be modified by both the [Client](https://agentclientprotocol.com/protocol/v1/session-modes#from-the-client) and the [Agent](https://agentclientprotocol.com/protocol/v1/session-modes#from-the-agent).
...
To start, we can implement this based on the [Session Modes](https://agentclientprotocol.com/protocol/v1/session-modes) api.
...
For updating the value from the client and agent, it would follow the same pattern as [session modes](https://agentclientprotocol.com/protocol/v1/session-modes#from-the-client) but have an additional key.
