+++
title = "rfds-v2-required-session-methods"
[extra]
skill = false
category = "tooling"
mermaid = false
...
to support the core session lifecycle methods: `session/list`,
...
support the lifecycle operations needed to create, discover, reconnect, replay,
...
The v2 schema should remove the optional `session.list`, `session.resume`, and
`session.close` capability fields. The [Session Resume Replay](@/skills/tooling/opencode-acp/resources/auto/rfds-v2-session-resume-replay.md)
RFD removes `session/load` from v2 and folds history replay into
`session/resume`.
