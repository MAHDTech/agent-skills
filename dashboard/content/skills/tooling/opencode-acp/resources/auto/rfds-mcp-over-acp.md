...
* An **[agent extension](@/skills/tooling/opencode-acp/resources/auto/rfds-proxy-chains.md)** that adds context-aware tools based on the conversation state
...
We've implemented this bridging as part of the conductor described in the [Proxy Chains RFD](@/skills/tooling/opencode-acp/resources/auto/rfds-proxy-chains.md). The conductor always advertises `mcpCapabilities.acp: true` to its clients, handling the translation transparently regardless of whether the downstream agent supports native ACP transport.
...
Agents advertise MCP-over-ACP support via the [`mcpCapabilities`](https://agentclientprotocol.com/protocol/v1/schema#mcpcapabilities) field in their `InitializeResponse`. We propose adding an `acp` field to this existing structure:
...
See the [Proxy Chains RFD](@/skills/tooling/opencode-acp/resources/auto/rfds-proxy-chains.md) for details on how MCP-over-ACP enables context-aware tooling.
