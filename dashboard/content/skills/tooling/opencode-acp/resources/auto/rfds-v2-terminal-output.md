...
A cross-version adapter must reject terminal content, v2 terminal session updates, and command permission subjects unless it implements the required stateful behavior itself. A v1-to-v2 bridge can only adapt a v1 terminal by implementing the v1 Client terminal lifecycle. A v2-to-v1 bridge may accumulate and sanitize terminal bytes into ordinary text content or use an implementation-specific extension, but it must not synthesize a v1 terminal resource it does not own.
