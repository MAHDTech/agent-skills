...
Tool calls represent actions that language models request Agents to perform during a [session](https://agentclientprotocol.com/protocol/v2/prompt-lifecycle). When an LLM determines it needs to interact with external systems—like reading files, running code, or fetching data—it generates tool calls that the Agent executes on its behalf.

Agents report tool calls through [`session/update`](https://agentclientprotocol.com/protocol/v2/prompt-lifecycle#3-agent-reports-output) notifications, allowing Clients to display real-time progress and results to users.
...
If the current active work gets [cancelled](https://agentclientprotocol.com/protocol/v2/prompt-lifecycle#cancellation), the Client **MUST** respond with the `"cancelled"` outcome:
...
<ResponseField name="outcome" type="RequestPermissionOutcome" required>
  ...
  cancelled](https://agentclientprotocol.com/protocol/v2/prompt-lifecycle#cancellation), and `selected` with an
...
Standard [content blocks](https://agentclientprotocol.com/protocol/v2/content) like text, images, or resources:
