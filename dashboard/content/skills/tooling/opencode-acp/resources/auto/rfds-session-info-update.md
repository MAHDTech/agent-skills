+++
title = "rfds-session-info-update"
[extra]
skill = false
category = "tooling"
mermaid = false
...
```typescript theme={null}
{
  ...
  _meta?: object | null             // Custom metadata update signal
}
```
...
2. **Create documentation** in `/docs/protocol/session-metadata.mdx`:
   ...
   * Document omitted, `null`, and object semantics for `_meta`
...
* Omitted `_meta` means no metadata update was supplied.
* `"_meta": null` is an explicit clear signal.
* `"_meta": { ... }` supplies a metadata object for clients and extensions to interpret.
...
Clients should process updates in order and apply each field according to its field semantics.
...
Yes, `_meta` is an arbitrary object. Agents define its structure.
