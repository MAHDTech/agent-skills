...
Proxies that provide MCP servers use the [MCP-over-ACP transport](@/skills/tooling/opencode-acp/resources/auto/rfds-mcp-over-acp.md) mechanism. The conductor always advertises `mcpCapabilities.acp: true` to proxies and handles bridging for agents that don't support native ACP transport.
...
* **MCP servers**: Proxies can provide tools via [MCP-over-ACP](@/skills/tooling/opencode-acp/resources/auto/rfds-mcp-over-acp.md) and handle tool callbacks
...
Proxies can provide MCP servers via [MCP-over-ACP transport](@/skills/tooling/opencode-acp/resources/auto/rfds-mcp-over-acp.md), enabling a single proxy to add context, provide tools, and handle callbacks with full awareness of the conversation state.
