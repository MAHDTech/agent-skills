...
* An **[agent extension](@/skills/tooling/opencode-acp/resources/rfds-proxy-chains.md)** that adds context-aware tools based on the conversation state
...
We've implemented this bridging as part of the conductor described in the [Proxy Chains RFD](@/skills/tooling/opencode-acp/resources/rfds-proxy-chains.md). The conductor always advertises `mcpCapabilities.acp: true` to its clients, handling the translation transparently regardless of whether the downstream agent supports native ACP transport.
...
See the [Proxy Chains RFD](@/skills/tooling/opencode-acp/resources/rfds-proxy-chains.md) for details on how MCP-over-ACP enables context-aware tooling.
