...
![](/hero.png) \`\`\` You can comma separate multiple meta attributes:
\`\`\`html ![Hero Alt Text](/hero.png) \`\`\` This will produce the
...
\`\`\`html [Hello World](/ "Homepage") \`\`\` This will generate the
...
Weights\](/docs/ranking/#configuring-metadata-weights) for details on
