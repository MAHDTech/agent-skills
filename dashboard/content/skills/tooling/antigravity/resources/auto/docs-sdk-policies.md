...
The SDK provides a declarative policy engine for controlling tool
execution and requesting human approval.

By default, custom Python functions and standard read-only tools are
allowed, while high-risk system operations (such as executing shell
commands via `run_command`) require explicit permission or policy rules.

## Declarative policy engine

Define explicit rules to allow, deny, or request human approval for tool
calls.

For example, you can configure the SDK to require approval when an agent
...
In CLI applications, you can prompt the user interactively before
high-risk tools are executed.

For example, use `run_interactive_loop` combined with
`policy.safe_defaults` to start a console session that prompts for user
approval whenever any write tool (file creation, editing, or shell
