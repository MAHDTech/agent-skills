...
- **Custom Python Functions:** Register any Python callable as an agent
  tool.
- **MCP Servers:** Connect any Model Context Protocol (MCP) server
...
- **Inspect** (Read-Only, Non-Blocking): For logging, audit trails, and
  metrics.
- **Decide** (Read-Only, Blocking): For custom approval/denial logic
  (policies).
- **Transform** (Modifying, Blocking): For sanitizing data in transit or
...
- **Streaming:** Access live model reasoning and output chunks as they
  are generated.
- **Multimodal Input:** Pass images, PDFs, audio, and video natively
  using `from_file()`.
- **Sub-agents:** Spawn child agents with independent tools and contexts
  to build multi-agent teams.
- **Structured Output:** Define schemas using Pydantic models to return
  validated, typed data directly.
- **Human-in-the-Loop:** Pause execution to ask structured questions and
  branch based on user input.
- **Observability:** Track per-turn and cumulative token usage and
  access thinking traces.

To use the SDK more easily within Antigravity 2.0, use the Antigravity
