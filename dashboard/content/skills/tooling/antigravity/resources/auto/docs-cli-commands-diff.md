...
- Use J/K (or ←/→) to quickly swap between files without returning to
  the list.
...
Press Esc to return to the file list. Press Esc again to exit `/diff`.
If you have unsaved comments, a confirmation screen appears:

- Press Shift + Y to approve and exit. Your comments are formatted and
  sent to the agent as your next prompt, allowing you to steer its next
  turn.
