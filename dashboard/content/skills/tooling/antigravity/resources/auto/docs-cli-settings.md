...
The CLI leverages **sparse persistence** by writing only values to disk
...
1.  Type `/config` (or its alias `/settings`) inside the prompt panel
    and press `Enter`.
2.  The full-screen **Settings Editor Overlay** opens.
3.  Navigate between available options using `↑`/`↓`.
4.  Press `Enter` on a highlighted parameter to toggle its state or open
    a text insertion field.
...
You can still edit the persistent value on disk during these sessions,
...
- **Key features**: Integrated scrollback, mouse-wheel scrolling
  support, custom rendered scrollbar, and clean terminal state
  restoration on exit.
...
- **Sandbox Mode (`enableTerminalSandbox`)**: When enabled (`on`),
  ...
  container.
- **Non-Workspace Access (`allowNonWorkspaceAccess`)**: Controls whether
...
- **Color Scheme (`colorScheme`)**: Selects the visual theme. Options
  ...
  variants.
- **Animation Speed (`runningLightSpeed`)**: Adjusts the speed of the
  progress indicator animation (`fast`, `medium`, `slow`, or `off`).
- **Verbosity (`verbosity`)**: Controls detail level. `high` shows full
...
- **Show Tips (`showTips`)**: Toggles the display of helpful usage tips
  while the agent is generating responses.
...
- **[Status Line Customization](https://antigravity.google/docs/cli/commands/statusline)**: Learn
  how to manage the status indicator panel and construct custom
  formatted status line shell scripts.
- **[Terminal Title Customization](https://antigravity.google/docs/cli/commands/title)**: Learn
...
To completely disable a default hotkey, map its action to an empty array
`[]`. If your JSON schema is malformed or invalid, the CLI falls back to
...
- **[Plugins & Skills](https://antigravity.google/docs/cli/plugins)**: Create your own custom
  skills and import legacy plugins.
