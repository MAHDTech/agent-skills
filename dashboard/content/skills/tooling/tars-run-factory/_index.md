...
- Use the packaged [supervisor](@/skills/tooling/tars-run-factory/resources/manual/supervisor.md) for every host turn; do not substitute sampled tails or an unmonitored print-mode fallback.
...
4. Read the [supervisor control protocol](@/skills/tooling/tars-run-factory/resources/manual/supervisor.md), then start `bun factory.ts <workspace_root>` from that resource directory with this shift's scope and limits.
...
- The [control reference](@/skills/tooling/tars-run-factory/resources/manual/supervisor.md) specifies interruption recovery, limits, handover and the code/foreman enforcement boundary.
