...
- **Do NOT Use Ephemeral Nix Shells for:**
  ...
  - If a repository utilizes `devenv`, project-level tools belong in `devenv.nix` (see the [devenv](@/skills/tooling/devenv/_index.md) skill). Never use `nix shell` as a permanent bypass for repository build environments.
